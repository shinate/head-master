...
headMaster.prototype.pack = function (source, namespace, opts) {
    ...
    this.dependList.forEach(function (name) {
        s.push('\'' + name + '\': ' + this.addPartWrapper(this.sourceTree[name]));
    }.bind(this));
...
headMaster.prototype.packAll = function (opts) {
    ...
    function parser(t) {
        ...
        if (t.type === 'directory' && t.children.length) {
            ...
            console.log(path.join(path.dirname(t.path), path.basename(t.path, '.js')));
            fs.writeFileSync(
...
headMaster.prototype.addBoxWrapper = function (content, packageName, type) {
    var headers = [];
    // AMD
    type & 4 > 0 && headers.push($T(this.getTpl('AMD-box-header'), {
        NAME: packageName
    }));
    // CMD
    type & 2 > 0 && headers.push(this.getTpl('CMD-box-header'));

    // GLOBAL
    type & 1 > 0 && headers.push($T(this.getTpl('GLOBAL-box-header'), {
...
headMaster.prototype.buildDependList = function (source, ns) {
    ...
    if (util.isString(source) && source !== '') {
        ...
        if (!(ns in this.sourceTree)) {
            this.sourceTree[ns] = this.stripComments(content).replace(/require\(([\"\']{1})([^\)]+)\1\)/g, function () {
                var dep = arguments[2];
                // get depend "name"
                if (/^\.{1,2}\//.test(dep)) {

...
headMaster.prototype.packFile = function (source, ns) {
    if (this.packageRealPath(source)) {
        fs.writeFileSync(
            this.mkFile(path.join(this.options.outputDir, source + '.js')),
            new Buffer(this.pack(source, ns || null))
        );
    }
};

headMaster.prototype.packDir = function (opts) {
...
headMaster.prototype.packAll = function (source, namespace, options) {
...
headMaster.prototype.buildDependList = function (source, ns) {
    ...
    if (util.isString(source) && source !== '') {
        ...
        if (ns == null) {
            throw new Error('Package "' + source + '" not exists!');
        }

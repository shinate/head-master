...
HMP.organize = function () {
    ...
    // variablify
    if (self.options.variablify) {

        self.contents = self.tpl('variablify-wrap', {
            PACKAGES: self.variablname(self.dependList.join(', ')),
            CONTENT: self.indentCharacter(self.dependList.map(function (name) {
                return self.tpl('variablify-part-wrap', {
                    NAME: self.variablname(name),
                    DEPENDS: self.variablname(self.sourceTree[name].deps.join(', ')),
                    CONTENT: self.indentCharacter(self.sourceTree[name].contents.replace(/require\(([\"\']{1})([^\)]+)\1\)/g, function (dep) {
                        return self.variablname(arguments[2]);
                    }))
                });
            }).join('\n\n')),
            EXPORT: self.variablname(self.dependList.slice(-1)[0])
        });
    } else {
        self.contents = self.tpl('require-wrap', {
            EXPORT: self.dependList.slice(-1)[0],
            CONTENT: self.indentCharacter(self.dependList.map(function (name) {
                return self.tpl('require-part-wrap', {
                    NAME: name,
                    CONTENT: self.indentCharacter(self.sourceTree[name].contents)
                });
            }).join(',\n'), 2)
        });
    }
...
HeadMaster.map = function (opts) {
    ...
    if (!!opts.bundle) {
        return (function () {
            ...
            return through.obj(function (file, env, cb) {
                ...
                source.forEach(function (p) {
                    _HM.walker(p.package, p.contents);
                });

                _HM.pack();

                console.log(_HM.contents);
            });
        ...
        return through.obj(function (file, env, cb) {
            ...
            file.contents = new Buffer(new HeadMaster(extend({
                base: path.relative(path.resolve('./'), file.base),
                contents: file.contents.toString(),
                package: path.join(path.dirname(_p), path.basename(_p, '.js'))
            }, opts)).pack().contents, 'utf8');
